...
x_L     = [-1e2;-1e-2;-1;0];           % lower bound (only for the nested logit, to avoid numerical problems when pho gets negative)
x_U     = [1e2;1.1e-2;1;0.975];       % upper bound (only for the nested logit, to avoid numerical problems when pho gets close to 1)
[thetaRCNL2S, FctValRCNL2S, exitflagRCNL2S,~,~] = ...
    knitromatlab(obj_fun,thetaRCNL, [], [], [], [], x_L, x_U, [], [],options, 'knitroBLP.opt');
% First Stage Results
meanval_FINAL2S=deltaNL_PAR(thetaRCNL2S,Data);
theta12S=(x1'*IV * W * IV'*x1)\(x1'*IV * W * IV'*meanval_FINAL2S);
gmmresid = meanval_FINAL2S - x1*theta12S;

save([data_dir 'gmmresid.mat'],'gmmresid');
save([data_dir 'mvaloldNL.mat'],'meanval_FINAL2S');


th12RCNL2S      = [theta1 ; abs(thetaRCNL2S)];
...
[ElaRCNL,DiversionRCNL] = ElastNestedLogit(alpha,thetaRCNL,meanval_FINAL2S,priceinc,Data);
