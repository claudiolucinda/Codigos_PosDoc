...
tam=0;
for i=1:max(cdid)
    tam=tam+(size(cdid(cdid==i,:),1)).^2;
end

projmat=spalloc(size(cdid,1),size(cdid,1),tam);
for i=1:max(cdid)
    warning('off','MATLAB:SingularMatrix');
    pmat=sparse(pmat_b(cdid==i,:));
    miolo=(pmat'*pmat)\speye(size(pmat,2));
    projmat(cdid==i,cdid==i)=speye(size(pmat,1))-pmat*miolo*pmat';
...
mean_inv=projmat*other(:,end-1);
cond_sh=other(:,end);
regrs=projmat*[ones(size(cond_sh)) regressors(:,2:end) cond_sh];
z=other(:,1:6);
incl_inst=[ones(size(cond_sh)) regressors(:,2:end-1)];
IV=projmat*[incl_inst z];
...
% Setting up stuff
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
x2=projmat*[regressors(:,2) regressors(:,3) regressors(:,4) cond_sh]; 
x1=projmat*[ones(size(cond_sh)) regressors(:,3:end-1)];
s_jt=regressors(:,1);
ns=250;
nobs=size(x1,1);
...
% Testing OK
Delta_Init=[regrs(:,1) regrs(:,3:end-1)]*[beta_IV_ini(1);beta_IV_ini(3:end-1)];
...
Data.sj=s_jt;
Data.lshare=projmat*(log(s_jt));
%meanval_test=testerNL_PAR(theta2_test,Data);
