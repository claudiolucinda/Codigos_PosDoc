...
for i=1:max(Data.cdid)
    ...
    partC(isnan(partC) | isinf(partC))  = 0;
    
    partD           = log(denom1t);
    partD(isnan(partD) | isinf(partD))  = 0;
    
    partE           = zeros(size(partD));
    for j=1:Data.ns
        temp=sum(unique(partD(:,j).*sgt(:,j)),1).* ones(size(numer1t,1),1);
        partE(:,j)=temp;
    end
    ...
    if isempty(dfull)==0;
        ...
        xd            = (x2(Data.cdid==i,l)*ones(1,ns)).*dfull(Data.cdid==i,(l-1)*ns+1:l*ns);
        part1         = xd./(1-Sigseg);
        part2A        = part1.*sijg(Data.cdid==i,:);
        part2         = (Sigseg).*(Big_selector*part2A);
        part2(isnan(part2) | isinf(part2))  = 0;
